namespace SheetXExample
{
	...
	public class LocalizationExampleText : MonoBehaviour
	{
	    ...
	    [SerializeField, HideInInspector] private LocalizationExample.ID m_LocalizedEnumId = LocalizationExample.ID.NONE; //Used to search for the usage of an ID within the project
	    ...
	    public LocalizationExample.ID LocalizedEnumId => m_LocalizedEnumId;
	    ...
	    {
	        if (LocalizationExample.CurLangIndex > -1)
	            Refresh();
			
			LocalizationExample.OnLanguageChanged += OnLanguageChanged;
	    }
	    ...
	    public void Set(int pId, params string[] pArgs)
	    {
	        ...
	        {
	            m_LocalizedID = pId;
	            m_LocalizedEnumId = (LocalizationExample.ID)pId;
	            m_LocalizedIDString = LocalizationExample.idString[m_LocalizedID];
	        }
			m_Args = pArgs;
			Refresh();
	    }
	    ...
	    public void Set(string pId, string[] pArgs)
	    {
	        if (m_LocalizedIDString != pId)
	        {
	            ...
	            m_LocalizedID = -1;
	            m_LocalizedEnumId = LocalizationExample.ID.NONE;
	        }
			m_Args = pArgs;
			Refresh();
	    }
	    ...
	    public void Refresh()
	    {
			int CurLangIndex = LocalizationExample.CurLangIndex;
	        if (Application.isPlaying && m_LanguageIndex == CurLangIndex)
				return;
			if (m_IgnoredLangs.Count > 0 && m_IgnoredLangs.Contains(LocalizationExample.CurrentLanguage))
				return;
	        string text = "";
	        if (m_LocalizedID >= 0)
	            text = LocalizationExample.Get(m_LocalizedID).ToString();
	        else if (!string.IsNullOrEmpty(m_LocalizedIDString))
	            text = LocalizationExample.Get(m_LocalizedIDString, ref m_LocalizedID).ToString();
	        switch (m_CaseType)
	        {
	    ...
	    private void OnValidate()
	    {
			...
			int localizedID = -1;
	        LocalizationExample.InitInEditor();
			LocalizationExample.Get(m_LocalizedIDString, ref localizedID);
			if (localizedID == -1)
			{
				string path = name;
				...
				Debug.LogError($"{path}: not found {m_LocalizedIDString} in {nameof(LocalizationExample)}");
			}
	    }
	
	    [CustomEditor(typeof(LocalizationExampleText))]
	    public class LocalizationExampleTextEditor : Editor
	    {
	        private LocalizationExampleText m_Target;
	        ...
	        private void OnEnable()
	        {
	            LocalizationExample.InitInEditor();
	            m_MaxResults = 20;
	            m_Target = target as LocalizationExampleText;
	        }
	        ...
	        public override void OnInspectorGUI()
	        {
				...
				if (LocalizationExample.CurrentLanguage != LocalizationExample.DefaultLanguage && GUILayout.Button("Reset Default Language"))
	            {
					LocalizationExample.CurrentLanguage = LocalizationExample.DefaultLanguage;
					m_Target.Refresh();
				}
				...
				if (m_Target.m_IgnoredLangs.Count > 0)
				{
					for (int i = 0; i < m_Target.m_IgnoredLangs.Count; i++)
					{
						...
						{
							m_IgnoredLangIdx--;
							m_IgnoredLangIdx = Mathf.Clamp(m_IgnoredLangIdx, 0, LocalizationExample.LanguageFiles.Count - 1);
							m_Target.m_IgnoredLangs[i] = LocalizationExample.GetLanguage(m_IgnoredLangIdx);
						}
						if (GUILayout.Button(">"))
						{
							m_IgnoredLangIdx++;
	            ...
	            {
	                int index = 0;
	                foreach (var lang in LocalizationExample.LanguageFiles)
	                {
	                    if (lang.Key == LocalizationExample.CurrentLanguage)
	                        break;
	                    index++;
	                }
	
	                index = (index + 1) % LocalizationExample.LanguageFiles.Count;
	                ...
	                foreach (var lang in LocalizationExample.LanguageFiles)
	                {
	                    if (index2 == index)
	                    {
	                        LocalizationExample.CurrentLanguage = lang.Key;
	                        m_Target.Refresh();
	                        break;
	                    }
	                    index2++;
	            ...
	            for (int i = 0; i < LocalizationExample.idString.Length; i++)
	            {
	                ...
	                bool contain = false;
	                string idString = LocalizationExample.idString[i];
	                ...
	                if (contain)
	                {
	                    string additional = "";
	                    if (m_ShowText)
	                        additional = LocalizationExample.Get(idString).ToString();
	                    ...
	                    if (GUILayout.Button(buttonName, style))
	                    {
	                        ...
	                        m_Target.m_LocalizedEnumId = LocalizationExample.ID.NONE;
	                        m_Target.Init();
	                        m_Target.Refresh();
	                    }
