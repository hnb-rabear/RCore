...
### 7.5. Structure preview and Data Class check

Every sheet table gains a **Structure** column with one button per ordinary data sheet: in `Excel Spreadsheets`, `Google Spreadsheets`, and both `Edit Spreadsheets` windows. The button reads the sheet and shows what it would export, without writing a file, creating a binding, or changing any setting — its tooltip says `Preview this sheet's C# structure without exporting files.` It appears whether or not Data Config Collections is enabled.

Sheets without a button: sheets whose name **ends in** `IDs`, `Constants`, or `Settings`, or **begins with** `Localization`, have no row structure to show — so `HeroIDs`, `ShopConstants`, `GameSettings`, and `LocalizationVN` are all excluded. Separately, a sheet named **exactly** `Configuration` is excluded because export writes it as a typed configuration class rather than a row array; that one is an exact-name match, so a longer name such as `ConfigurationExtra` is an ordinary sheet and does get a button.

The button opens the **Sheet Structure** window, which reports **Source**, **Sheet**, **Output Mode**, **Fetched** — the date and local time this snapshot was read — and **Data Class** at the top, then a **Top-level fields** table, then the C# code the sheet corresponds to. **Refresh** re-reads the sheet; **Copy Code** puts the class code on the clipboard — code only, never the JSON.

**What the code block means depends on Output Mode:**

- **`Generated Data Class`** is labelled **Generated class preview**. This is the real generated class: the class declaration is byte-identical to the one the next export writes, with no preview-only commentary added to it. The exported file itself is larger — it also carries the collection paths and every other table's row types.
- **`JSON Only` and `Existing Data Class`** are labelled **Inferred draft from exported JSON**. These modes declare no schema, so the window infers one from the JSON the exporter would actually produce. Treat it as a starting point, not a contract. The window states the three limits itself: a blank or omitted field is absent from the data, so it cannot appear here; inferred types are suggestions read from the values present, not declarations; and the ID scope depends on which tab opened the preview, as described next.

**Which IDs a preview resolves depends on the tab it was opened from**, because the two tabs run different exports. A preview opened from an **Export Single File** tab predicts that tab's own export, so symbolic IDs resolve against that one source; the window says `IDs are resolved against this source only, as this tab's own export does`. A preview opened from an **Export Multi Files** tab — including the per-source `Edit Spreadsheets` window its **Select** button opens — predicts `Export All`, so IDs resolve across that list, in list order, first definition winning; the window says `IDs are resolved against every source in the Export Multi Files list, as that export does`. The two platforms differ in which listed entries contribute IDs, and each preview matches its own export: Excel reads the `*IDs` sheets of every listed workbook regardless of the checkboxes, while Google reads only selected spreadsheets and their selected `*IDs` sheets. This governs ID resolution alone — an unselected source's data sheets are still never exported. One consequence on the Google side: previewing a sheet in a listed spreadsheet that is itself **unticked** keeps the single-spreadsheet scope, because `Export All` skips that spreadsheet outright; its own `*IDs` sheets still resolve, so the preview reads no fewer IDs than opening it alone would. The same file or spreadsheet can sit in both places, and it keeps each tab's own scope accordingly — previewing it from the Single tab does not borrow the list's IDs.

For Google, the multi-source ID map is built once and kept for the session, because it costs one connection plus one range read per `*IDs` sheet per listed spreadsheet. **Refresh** rebuilds it, which is how you pick up an IDs sheet edited on the web. A map is only remembered when it was built completely: if a listed spreadsheet could not be reached, the preview still shows its result with a diagnostic naming that spreadsheet, but the next preview walks the list again rather than reusing the gap. Sheets added to a listed spreadsheet on the web are discovered from its live metadata, so a newly created `*IDs` sheet resolves in preview as it does in export, without needing to reopen `Edit Spreadsheets` — an IDs sheet you explicitly unticked stays excluded.

The draft opens with a three-line comment header repeating that it is inferred rather than recovered, so the meaning travels with the code when you paste it elsewhere. Inside the class, only two kinds of field earn a comment: one whose JSON key could not be used as a C# identifier and was renamed, and one whose type fell back to `object` or `object[]`. A renamed field's comment names the exact **JSON key** — not a spreadsheet header, because the legacy Attribute System can fold several columns into one key and the original header cannot be recovered from the JSON. A fallback comment gives the reason inference itself recorded, such as `fallback to object: mixes null with a number` or `fallback to object[]: only empty arrays were observed`. Ordinary declarations carry no comment.

**Top-level fields table.** Above the code, a three-column table lists `JSON key`, `C# field`, and `Type` for the top level only. A `*` after the type marks an inferred fallback, with the reason in its tooltip. Nested objects become supporting classes and appear in the code below rather than in the table. In `Generated Data Class` mode the table is replaced by a one-line note pointing at the code, whose fields come from the sheet's declared schema.

The blank-cell rule is the one that surprises people. SheetX omits an empty cell from exported JSON entirely, so a column that is blank in **every** row does not appear in the draft at all, and a column blank in only some rows still produces a field. List the column under **Persistent columns** in Settings if you need it present regardless. The ID scope matters in the opposite direction, but only across tabs: a symbolic key left as text in a **Single** preview may resolve to a number under `Export All` from the **Multi Files** list, which shares one ID namespace. A **Multi Files** preview already resolves against that same list, so it does not differ from the export it predicts.

**`Class Code` and `JSON` tabs.** For `JSON Only` and `Existing Data Class` sheets the window offers two tabs: the inferred draft, and the exported JSON itself. Switching tabs re-reads nothing — the JSON is formatted once when the snapshot is taken, and date-like strings stay strings rather than being reformatted into something the export never wrote. `Generated Data Class` has no legacy JSON, so it shows no JSON tab at all. The view renders at most 50,000 characters: input already longer than that is shown as an unformatted excerpt labelled `Large JSON: showing a raw excerpt`, and input that only exceeds the limit once indented is cut with its own note. Either way the label says the excerpt is incomplete and will not parse as JSON on its own. Only the on-screen view is shortened — the stored JSON the member check validates against is always the whole document.

**Freshness.** The snapshot of the sheet you are previewing is taken when you open the window and belongs to that window alone — it is never cached, and pressing **Structure** again re-reads that sheet from scratch. The shared Google ID map described above is the one exception: it is reused for the session rather than re-read per preview, until **Refresh**, a domain reload, a different settings asset, or a change to the `Export Multi Files` list or its selection rebuilds it. The **Fetched** row in the header shows the date and local time of the snapshot on screen, so a window left open is never mistaken for a fresh read; a failed refresh keeps the old snapshot and its original timestamp. Press **Refresh** after editing the spreadsheet. If a refresh fails, the previous code stays on screen under `Stale — last refresh failed:` and the reason, so it is never presented as current. Changing a sheet's Output Mode while the window is open shows `Output Mode changed since this preview was taken. Press Refresh to reload it.`

**Member check for `Existing Data Class`.** When a sheet is bound to a class you wrote, the window adds an **Existing Data Class** block comparing the exported JSON keys against that class:

- **`Unmatched JSON member '<name>'`** (one warning per key) means the exported data carries a value the class has no member that accepts it, so that column's values are discarded when the JSON is loaded. The message says so and names the two fixes: add a matching member to the class, or rename the sheet column so it matches an existing one. Where the draft inferred a declaration for that key, the warning shows it as a **suggestion, not a guaranteed fix** — check it before pasting. A renamed key's suggestion includes its `JsonProperty` mapping, because the bare declaration would not receive the JSON key; an `object` or `object[]` fallback in particular is not something Unity serializes as-is.
- **`No unmatched top-level JSON members`** means every exported key has a destination.
- **`Not present in exported data: <names>`** is informational only. A blank cell is omitted from the export, so these members may simply have no values in this sheet right now. The message deliberately does not claim what each one holds after loading — that depends on the class, which may have constructor defaults.
- **`Member coverage cannot be verified`** keeps the specific reason: no row type selected, a custom `JsonConverter` (on the class or in the serializer settings), a non-object contract, `[JsonExtensionData]`, a read-only member the deserializer may populate in place, or an error while resolving the contract. It means the check can neither confirm nor deny coverage — not that anything is wrong with the data. Confirm the mapping by inspecting the class yourself.
- A **binding problem outranks the whole block** and replaces it with a single error. A bound type that no longer resolves is named — `Row type '<name>' was not found in any loaded assembly.` — and a type the export would reject keeps that rejection's own reason, such as `row type '<full name>' is missing [SheetXBindable].` Both end with `Export skips this sheet until this is fixed.`, because export resolves and validates the same type and skips the sheet. These are errors rather than the reassuring `cannot be verified` wording, which would be untrue here. A binding that names no class at all is not a problem — that arm reports `Member coverage cannot be verified: no row type is selected.` instead.
- A conversion error replaces the clean result when the member names match but a value's type does not (text in an `int` field, for example). Export performs the same conversion and would refuse the sheet, so the message points you at the named value and the declared type of the matching member. Nothing is fixed automatically.
- The check is **top-level only** this release: `Top-level member check only; nested members are not compared.` A missing member inside a nested object is not reported.

**The check follows the real deserializer.** It resolves the contract through `JsonSerializer.CreateDefault()` on every comparison, so a project that sets `JsonConvert.DefaultSettings` — a snake_case naming strategy, say — is honoured, including changes made after an earlier check ran. Where the outcome depends on the data rather than the type, the check says so instead of guessing: a get-only collection or nested object may or may not be populated in place depending on whether the getter returns a non-null instance, so such a type reports `Member coverage cannot be verified` rather than accusing the export of discarding data it keeps. A get-only **scalar** can never receive a value, so it is still reported as unmatched.

**Export confirmation (behavior change).** During a collection export, if any accepted `Existing Data Class` sheet carries exported members with no destination, SheetX now asks once — an **Existing Data Class members** dialog offering **Export Anyway** and **Cancel**. It is one aggregated question for the whole collection export, listing every offending source, sheet, class, and member name, and it is asked **before the collection export writes anything**.

- **Export Anyway** writes exactly what previous versions wrote; the JSON is unchanged.
- **Cancel** aborts the **collection** part of the export with `Collection export cancelled: unmatched Existing Data Class members were not confirmed.` No collection JSON and no generated collection source is written, so there is no half-written collection output to repair. Cancelling because of one sheet cancels the other collection sheets with it.
- **Cancel does not undo earlier stages of the same run.** The collection step runs after ordinary JSON, and in **Export All** after IDs and Constants as well. Those artifacts are already on disk when the dialog appears and stay there — `Cancel` stops the collection write, it does not roll the whole export back. Re-run the export after fixing the class or the column.
- Every finding is logged as a warning whether or not the dialog appears, so the Console keeps the full list even when the dialog truncates it.
- **Batch and headless runs never show this dialog.** The same findings are logged as warnings and the collection export continues. This applies to the member mismatch only — it does not make other aborts permissive. A storage-mode change still aborts a headless run with "was not confirmed" (section 7.4), and a sheet whose JSON does not deserialize into its row type is still skipped with an error. The detached `SheetXExporter` and batch APIs carry no collection bindings at all, so the check never arises there.

A column that has been quietly dropping data for months will now interrupt an export that previously ran clean. That is the intended effect: the dialog is the first report of a mismatch that was always silently discarding values. Either add the missing members to the class or rename the columns, and the prompt stops appearing.
