...
namespace RCore.SheetX.Editor
{
	...
	internal static class SheetXCollectionGenerator
	{
		...
		/// <summary>
		/// Emits the generated source, or throws <see cref="InvalidOperationException"/> on a name
		/// collision. Collisions are never resolved by suffixing — the generated API has to stay stable.
		/// </summary>
		internal static string Emit(SheetXSettings settings, IReadOnlyList<SheetXCollectionGeneratedTable> tables)
		{
			...
			bool hasNamespace = !string.IsNullOrWhiteSpace(collectionNamespace);
			string indent = hasNamespace ? T : "";
			if (hasNamespace)
			{
			...
			foreach (var table in ordered.Where(t => t.Schema != null))
				AppendRowTypes(source, indent, table.Schema);

			var collectionOrder = CollectionOrder(settings, ordered);
			foreach (string collection in collectionOrder)
				AppendCollection(source, indent, collection, collectionOrder, ordered);

			AppendPaths(source, indent, settings, ordered);
