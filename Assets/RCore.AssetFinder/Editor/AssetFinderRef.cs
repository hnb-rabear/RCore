...
//		}
        ...
        private static void FindRefInScene()
        {
            ...
            {
                var asset = AssetFinderCache.Api.Get(cacheAssetGuids[i]);
                if (asset == null)
                {
                    continue;
                }
            ...
            AssetFinderAsset asset)
        {
            ...
            {
                foreach (var item in AssetFinderUnity.getAllObjsInCurScene())
                {
            ...
            {
                foreach (var item in AssetFinderSceneCache.Api.folderCache[dir])
                {
                    if (AssetFinderSceneCache.Api.cache.ContainsKey(item))
                    {
                        foreach (var item1 in AssetFinderSceneCache.Api
                                     .cache[item])
                        {
    ...
    public class AssetFinderRef
    {
            ...
            params string[] guidList)
        {
            ...
            for (var i = 0; i < guidList.Length; i++)
            {
                ...
                var child = AssetFinderCache.Api.Get(guid);
                if (child == null)
                {
                    continue;
                }
        ...
        internal void AppendUsedBy(Dictionary<string, AssetFinderRef> result, bool deep)
        {
            ...
            var list = deep ? new List<AssetFinderRef>() : null;
            ...
            bool depth)
        {
            ...
            for (var i = 0; i < objs.Length; i++)
            {
                ...
                foreach (var item in AssetFinderUnity.GetAllRefObjects(objs[i]))
                {
                ...
                if (depth)
                {
                    foreach (var child in AssetFinderUnity.getAllChild(objs[i]))
                    foreach (var item2 in AssetFinderUnity.GetAllRefObjects(child))
                    {
    ...
    public class AssetFinderRefDrawer : IRefDraw
    {
        ...
        {
            var lst = AssetFinderCache.Api.ScanUnused();
