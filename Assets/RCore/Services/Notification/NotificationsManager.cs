...
namespace RCore.Notification
{
	...
	public class NotificationsManager : MonoBehaviour
	{
		...
		public enum OperatingMode
		{
			...
			/// No messages will be sent to the operating system until the application is in background.
			/// </para>
			/// <para>
		...
		private OperatingMode m_mode = OperatingMode.QueueClearAndReschedule;

		[SerializeField, Tooltip("Check to make the notifications manager automatically set badge numbers so that they increment.\n" + "Schedule notifications with no numbers manually set to make use of this feature.")]
		private bool m_autoBadging = true;
		...
		/// </summary>
		public NotificationsPlatform Platform { get; private set; }
		...
		{
			if (PendingNotifications == null || !PendingNotifications.Any() || (m_mode & OperatingMode.Queue) != OperatingMode.Queue)
				return;
			...
			for (int i = PendingNotifications.Count - 1; i >= 0; --i)
			{
				...
				var time = queuedNotification.DeliveryTime;
		...
		protected void OnApplicationFocus(bool hasFocus)
		{
			...
			foreach (var pendingNotification in PendingNotifications)
			{
				...
				if ((m_mode & OperatingMode.ClearOnForegrounding) == OperatingMode.ClearOnForegrounding)
				{
					...
					if (pendingNotification.Reschedule && pendingNotification.Scheduled)
		...
		public IEnumerator IEInit()
		{
			...
			Platform = new NotificationsPlatform(args);

			PendingNotifications = new List<PendingNotification>();
			Platform.NotificationReceived += OnNotificationReceived;

			// Check serializer
			Serializer ??= new DefaultSerializer(Path.Combine(Application.persistentDataPath, DEFAULT_FILENAME));
		...
		public void SendNotification(string title, string body, DateTime deliveryTime, int? badgeNumber = null, bool reschedule = false, string data = null)
		{
			...
			Debug.Log($"Queued Notification ID \"{notification.Id}\" at time {deliveryTime:HH:mm}");
		}
		...
		public void CancelNotification(int notificationId)
		{
			...
			int index = PendingNotifications.FindIndex(notification => notification.Notification.Id == notificationId);
		...
		private void OnNotificationReceived(GameNotification deliveredNotification)
		{
			...
			int deliveredIndex = PendingNotifications.FindIndex(scheduledNotification => scheduledNotification.Notification.Id == deliveredNotification.Id);
			if (deliveredIndex >= 0)
			{
		...
		private void OnForegrounding()
		{
			...
			if ((m_mode & OperatingMode.ClearOnForegrounding) == OperatingMode.ClearOnForegrounding)
			{
				...
				if (loaded == null || (m_mode & OperatingMode.RescheduleAfterClearing) != OperatingMode.RescheduleAfterClearing)
					return;
				...
				foreach (var savedNotification in loaded)
					...
					{
						var pendingNotification = ScheduleNotification(savedNotification.Notification, savedNotification.DeliveryTime);
