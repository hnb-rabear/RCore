...
namespace RCore.Editor.Tool
{
	...
	public class UIChangeButtonsColorTool : RCoreHubTool
	{
		public override string Name => "Change Buttons Transition Color";
		public override string Category => "UI Tools";
		public override string Description => "Batch-change transition colors for a list of Button components.";
		public override bool IsQuickAction => false;
		...
		public override void DrawFocusMode()
		{
			if (Selection.gameObjects == null || Selection.gameObjects.Length == 0)
			{
				EditorGUILayout.HelpBox("Select GameObjects that have a Button component", MessageType.Warning);
				return;
			}
			...
			if (m_Buttons != null && m_Buttons.Count > 0)
			{
				EditorHelper.BoxHorizontal(() =>
				{
					...
					if (m_ColorBlocksForChange[0]) m_ButtonColors.normalColor = EditorHelper.ColorField(m_ButtonColors.normalColor, "", 0, 100);
				});

				EditorHelper.BoxHorizontal(() =>
				{
					m_ColorBlocksForChange[1] = EditorHelper.Toggle(m_ColorBlocksForChange[1], "Pressed Color", 150, 23);
					if (m_ColorBlocksForChange[1]) m_ButtonColors.pressedColor = EditorHelper.ColorField(m_ButtonColors.pressedColor, "", 0, 100);
				});

				EditorHelper.BoxHorizontal(() =>
				{
					m_ColorBlocksForChange[2] = EditorHelper.Toggle(m_ColorBlocksForChange[2], "Highlighted Color", 150, 23);
					if (m_ColorBlocksForChange[2]) m_ButtonColors.highlightedColor = EditorHelper.ColorField(m_ButtonColors.highlightedColor, "", 0, 100);
				});

				EditorHelper.BoxHorizontal(() =>
				{
					m_ColorBlocksForChange[3] = EditorHelper.Toggle(m_ColorBlocksForChange[3], "Disabled Color", 150, 23);
					if (m_ColorBlocksForChange[3]) m_ButtonColors.disabledColor = EditorHelper.ColorField(m_ButtonColors.disabledColor, "", 0, 100);
				});
				...
				if (EditorHelper.ButtonColor("Change Colors", Color.yellow))
				{
					foreach (var buttons in m_Buttons)
					{
						foreach (var button in buttons.Value)
						{
							...
							if (m_ColorBlocksForChange[0]) colors.normalColor = m_ButtonColors.normalColor;
							if (m_ColorBlocksForChange[1]) colors.pressedColor = m_ButtonColors.pressedColor;
							if (m_ColorBlocksForChange[2]) colors.highlightedColor = m_ButtonColors.highlightedColor;
							if (m_ColorBlocksForChange[3]) colors.disabledColor = m_ButtonColors.disabledColor;
	...
	public class UIChangeTextsFontTool : RCoreHubTool
	{
		public override string Name => "Change Unity Texts Font";
		public override string Category => "UI Tools";
		public override string Description => "Batch-change the Font for legacy Unity Text components.";
		public override bool IsQuickAction => true;
		...
		public override void DrawCard()
		{
			...
			m_Font = (Font)EditorHelper.ObjectField<Font>(m_Font, "Font", 50);
			if (m_Font != null && EditorHelper.ButtonColor("Apply Font", Color.cyan))
			{
				var m_Texts = EditorHelper.FindComponents<Text>(objs, null);
				if (m_Texts != null)
				{
