...
namespace RCore.Components
{
    ...
    public class HybridAudioManager : BaseAudioManager
    {
        ...
        public void PlaySFX(string pFileName, bool pLoop = false, float pPitchRandomMultiplier = 1)
        {
            ...
            int outIndex = -1;
            var clip = HybridAudioCollection.Instance.GetClip(pFileName, false, out outIndex);
            if (clip != null)
        ...
        public void PlaySFXById(int pId, bool pLoop = false, float pPitchRandomMultiplier = 1)
        {
            if (!m_EnabledSFX) return;
            int outIndex = -1;
            var clip = HybridAudioCollection.Instance.GetClipById(pId, false, out outIndex);
            if (clip != null)
                PlaySFX(clip.clip, clip.limitNumber, pLoop, pPitchRandomMultiplier);
        }
        ...
        {
            int outIndex = -1;
            var clip = HybridAudioCollection.Instance.GetClip(pFileName, true, out outIndex);
            if (clip != null)
        ...
        {
            int outIndex = -1;
            var clip = HybridAudioCollection.Instance.GetClipById(pId, true, out outIndex);
            if (clip != null)
        ...
        protected class HybridAudioManagerEditor : BaseAudioManagerEditor
        {
