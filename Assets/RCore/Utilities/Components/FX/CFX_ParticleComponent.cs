...
namespace RCore.Components
{
	...
	public class CFX_ParticleComponent : CFX_Component
	{
		...
		{
			if (pRadius is CapsuleCollider)
				transform.localScale = Vector3.one * (pRadius as CapsuleCollider).radius / radius;
			else if (pRadius is SphereCollider)
				transform.localScale = Vector3.one * (pRadius as SphereCollider).radius / radius;
			else if (pRadius is BoxCollider)
				transform.localScale = Vector3.one * ((pRadius as BoxCollider).size.x + (pRadius as BoxCollider).size.z) / 3 / radius;
		}
