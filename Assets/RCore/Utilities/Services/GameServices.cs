...
namespace RCore.Service
{
    ...
    public partial class GameServices : MonoBehaviour
    {
        ...
        private bool isLoadingScore = false;
        ...
        public void Init(Action<bool> pOnAuthenticated)
        {
            ...
            if (settings.GpgsOauthScopes != null && settings.GpgsOauthScopes.Length > 0)
            {
                foreach (string scope in settings.GpgsOauthScopes)
                    gpgsConfigBuilder.AddOauthScope(scope);
            }

            // Request ServerAuthCode if needed.
            if (settings.GpgsShouldRequestServerAuthCode)
                gpgsConfigBuilder.RequestServerAuthCode(settings.GpgsForceRefreshServerAuthCode);
            ...
            PlayGamesPlatform.DebugLogEnabled = settings.GgpsDebugLogEnabled;
        ...
        {
            foreach (Leaderboard ldb in settings.Leaderboards)
            {
        ...
        {
            foreach (Achievement acm in settings.Achievements)
            {
...
#if UNITY_ANDROID && GPGS
                PlayGamesPlatform.Instance.SetGravityForPopups(ToGpgsGravity(settings.GpgsPopupGravity));
#endif
            }
...
#if UNITY_ANDROID && GPGS
        private Gravity ToGpgsGravity(Settings.GpgsGravity gravity)
        {
            switch (gravity)
            {
                case Settings.GpgsGravity.Top:
                    return Gravity.TOP;
                case Settings.GpgsGravity.Bottom:
                    return Gravity.BOTTOM;
                case Settings.GpgsGravity.Left:
                    return Gravity.LEFT;
                case Settings.GpgsGravity.Right:
                    return Gravity.RIGHT;
                case Settings.GpgsGravity.CenterHorizontal:
