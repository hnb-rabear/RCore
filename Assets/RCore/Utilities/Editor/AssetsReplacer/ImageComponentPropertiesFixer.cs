...
namespace RCore.Editor
{
	...
	public class ImageComponentPropertiesFixer
	{
		...
		private JustButton.PerfectRatio m_perfectRatio;
		
		public bool CanSubmit()
		{
			return m_useSpriteMesh != YesNoNone.None || m_raycastTarget != YesNoNone.None || m_maskable != YesNoNone.None || m_perfectRatio != JustButton.PerfectRatio.None;
		}
					...
					foreach (var target in m_targets)
					{
						...
						foreach (var image in images)
						{
							if (m_sprites.Contains(image.sprite))
							{
								...
								if (m_perfectRatio == JustButton.PerfectRatio.Height)
									RUtil.PerfectRatioImageByHeight(image);
								else if (m_perfectRatio == JustButton.PerfectRatio.Width)
