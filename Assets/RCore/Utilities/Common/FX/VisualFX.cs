...
namespace RCore.Common
{
	public class VFXBase
	{
		...
		protected WaitForSeconds m_WaitForDeactive = null;
		protected Coroutine m_DeactiveCoroutine;
		...
		public void AutoDeactive(float pCustomTime = 0)
		{
			if (m_DeactiveCoroutine != null)
				CoroutineUtil.StopCoroutine(m_DeactiveCoroutine);

			if (pCustomTime != 0)
				m_DeactiveCoroutine = CoroutineUtil.StartCoroutine(iEAutoDeactive(new WaitForSeconds(pCustomTime)));
			else
				m_DeactiveCoroutine = CoroutineUtil.StartCoroutine(iEAutoDeactive(m_WaitForDeactive));
		}
